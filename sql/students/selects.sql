CREATE OR REPLACE PROCEDURE student_id_select(id UUID)
LANGUAGE SQL
as $$
BEGIN
...
CREATE OR REPLACE PROCEDURE student_skipped_lessons(user_id UUID) 
LANGUAGE SQL
as $$
BEGIN
    SELECT l.lesson_start, l.lesson_end, p.name, p.lesson_type, s.skipped_hours FROM skipped_hours as s INNER JOIN lessons as l ON s.skipped_lesson_id=l.id 
    INNER JOIN lessons_prototypes as p ON p.id=l.prototype_id WHERE s.student_id=user_id;
END
$$;
